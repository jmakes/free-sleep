...
Awake pass (jmakes.10): sleep-baseline HR from quiet epochs, sleep-onset
latency, and quiet/restless-mind elevated-HR wake.
"""
...
def _sleep_baseline_hr(features: Sequence[Dict[str, Any]]) -> float:
    quiet = [
        float(f['hr'])
        for f in features
        if f['has_vitals'] and f['hr'] > 0 and f['movement_max'] < MOVEMENT_QUIET
    ]
    if len(quiet) >= MIN_QUIET_BASELINE_EPOCHS:
        return float(median(quiet))
...
def _sleep_baseline_br(features: Sequence[Dict[str, Any]]) -> float:
    quiet = [
        float(f['br'])
        for f in features
        if f['has_vitals'] and f['br'] > 0 and f['movement_max'] < MOVEMENT_QUIET
    ]
    if len(quiet) >= MIN_QUIET_BASELINE_EPOCHS:
        return float(median(quiet))
...
def _is_asleep_like(
    ...
    if not has_vitals or baseline_hr <= 0:
        return movement_max < MOVEMENT_QUIET
