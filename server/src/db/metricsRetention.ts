...
export async function getFreeDiskMb(targetPath = config.dbFolder): Promise<number | undefined> {
  try {
    // Node 18.15+ / 19+
...
export async function getMetricsStats() {
  ...
  const [vitalsCount, movementCount, sleepCount, oldestVital, newestVital, oldestMovement, newestMovement] =
    await Promise.all([
      prisma.vitals.count(),
...
): { gte: number; lte: number; defaulted: boolean; clamped: boolean } {
