...
export type GestureFieldSnapshot = {
  timestamp: string;
  allKeys: string[];
  /** Keys that look gesture-related (for discovery) */
  tapLikeKeys: Record<string, string>;
  /** Normalized counters we extracted */
...
function parseSideCounters(raw: string): { l: number; r: number } | null {
  try {
...
export function extractGestureCounters(rawMap: Record<string, string>): GestureFieldSnapshot {
  ...
  for (const side of ['left', 'right'] as Side[]) {
    if (left.singleTap !== undefined && side === 'left') continue;
    if (right.singleTap !== undefined && side === 'right') continue;
  ...
  if (left.singleTap === undefined && right.singleTap === undefined) {
    for (const [key, value] of Object.entries(tapLikeKeys)) {
      ...
      if (/double|triple|quad|multi/i.test(key)) continue;
  ...
  if (!loggedDiscovery) {
    ...
    logger.info(
      `Gesture field discovery: tapLikeKeys=${JSON.stringify(tapLikeKeys)} resolved=${JSON.stringify(resolvedAliases)}`
    );
    if (!resolvedAliases.singleTap) {
      logger.warn(
        'No single-tap field found in DEVICE_STATUS. Single-tap actions will not fire until firmware exposes a counter. ' +
