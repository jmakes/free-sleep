...
export type PoseAction = 'capture' | 'status' | 'finalize' | 'reset';
...
function resolvePoseScript(): string {
  const candidates = [
...
function resolveBiometricsCwd(script: string): string {
...
export async function runPoseCalibration(args: {
  ...
  try {
    ...
    const anyErr = error as { stdout?: string };
    if (anyErr.stdout) {
      try {
 ...
 * on nested objects.
 */
function parsePoseJson(stdout: string, action: string): PoseCalibrationResult {
  ...
  return JSON.parse(trimmed.slice(jsonStart, jsonEnd + 1)) as PoseCalibrationResult;
}
