...
export function getSensorThresholds(side) {
    const filePath = baselinePath(side);
    ...
    const manualOverrideAt = typeof profile?.manual_override_at === 'string' ? profile.manual_override_at : null;
    const personalized = Boolean(profile?.piezo_range_threshold != null || profile?.poses || source === 'manual');
    return {
        side,
        exists,
        path: filePath,
 ...
 */
export function patchSensorThresholds(side, patch) {
    const filePath = baselinePath(side);
    const existing = readProfile(side);
    if (!existing) {
        throw new Error(`No ${side} calibration baseline yet. Run guided Sensors calibration or empty-bed cal first.`);
    }
    const next = { ...existing };
    ...
    if (!next.cap_method)
        next.cap_method = 'max_z';
    const folder = path.dirname(filePath);
    if (!existsSync(folder))
        mkdirSync(folder, { recursive: true });
    writeFileSync(filePath, `${JSON.stringify(next, null, 4)}\n`, 'utf8');
    logger.info(`Manual threshold override for ${side}: cap_z=${next.cap_zone_threshold} piezo=${next.piezo_range_threshold}`);
    return getSensorThresholds(side);
}
//# sourceMappingURL=sensorProfile.js.map