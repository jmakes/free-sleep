...
let started = false;
let running = false;
async function runPrune(reason) {
    if (running) {
...
export function startMetricsRetentionJob() {
    if (started)
        return;
    started = true;
    logger.info(`Scheduling metrics retention: first run in ${Math.round(FIRST_RUN_DELAY_MS / 1000)}s, then every ${Math.round(INTERVAL_MS / 3600000)}h`);
    setTimeout(() => {
...
//# sourceMappingURL=metricsRetentionJob.js.map