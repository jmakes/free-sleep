...
function classifyEpoch(args: {
  ...
  if (!args.hasVitals || args.medHr <= 0) {
  ...
  // At-or-below night median counts as "low" so flat quiet nights can still be Deep.
  const hrLow = args.hr > 0 && args.hr <= args.medHr;
  const hrHigh = args.hr >= args.medHr * 1.05;
  const hrvHigh = args.hrv >= args.medHrv * 1.08 && args.medHrv > 0;
  const hrvLow = args.hrv > 0 && args.medHrv > 0 && args.hrv <= args.medHrv * 0.95;
  const brLow = args.br > 0 && args.medBr > 0 && args.br <= args.medBr * 1.02;

  if (restless && (hrHigh || args.hr === 0)) {
  ...
  if ((quiet || stirring) && hrHigh && hrvHigh && !restless) {
...
export function computeStageV1(args: {
  ...
  const medHr = median(vitals.map((v) => v.hr).filter((v) => v > 0));
  const medHrv = median(vitals.map((v) => v.hrv).filter((v) => v > 0));
  const medBr = median(vitals.map((v) => v.br).filter((v) => v > 0));

  const raw: StageEpoch[] = [];
  for (let startMs = nightStart; startMs < nightEnd; startMs += EPOCH_MS) {
    ...
    raw.push({ startMs, endMs, stage, reasons });
  }

  const epochs = smoothStages(raw);
